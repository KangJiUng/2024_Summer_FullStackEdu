...
// node.js backend에서는 require 예약어를 이용해 설치된 패키지를 참조합니다.
const moment = require("moment");

// 환경변수 파일에서 환경변수를 가져오기위해 dotenv를 참조합니다.
const env = require("dotenv");

// 프로젝트 루트에 있는 .env 파일에 환경변수 정보를 CPU프로세스에서 접근가능하게 구성합니다.
env.config();
...
console.log("현재 일자를 출력합니다. 순수 자바스크립트1:", toDate);

console.log("현재 일자를 출력합니다. 순수 자바스크립트2:", toDate2);

// moment 패키지를 통해 숫자 타입 날짜 데이터 포맷을 변경합니다.
var formattedDate = moment(toDate2).format("YYYY-MM-DD HH:mm:ss");

// 대부분의 자바스크립트 오류는 오탈자입니다.
// 초기 자바스크립트 언어개발 시 문제가 있다면 오탈자/대소문자 문제입니다.
// 자바스크립트는 대소문자를 가려요.
