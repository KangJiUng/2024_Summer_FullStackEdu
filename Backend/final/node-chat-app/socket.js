...
module.exports = (server) => {
  ...
  // input 메시지는 웹브라우저에서 들어오는 메시지, output 메시지는 서버소켓에서 웹브라우저로 전송하는 메시지
  ...
  io.on("connection", (socket) => {
    ...
    // 웹브라우저(클라이언트/프론트엔드)에서 서버소켓에 broadcast 이벤트를 호출하면
    // 서버소켓이 클라이언트에서 보내준 메시지를 수신하고 콜백함수를 통해 클라이언트로 메시지를 전송(io.emit())합니다.
    socket.on("broadcast", function (msg) {
      // 현재 메시지 서버에 연결된 모든 사용자들(웹브라우저/프론트엔드웹페이지)에게
      // 메시지를 전송하는데, 클라이언트 메시지 수신 이벤트명이 receiveAll로 msg데이터를 전송한다.
      // io.emit() 메서드는 서버소켓(io)에 연결된 모든 사용자에게 메시지를 보낼 때 사용
      // io.emit("클라이언트 이벤트수신기명", 클라이언트로 보낼 데이터);
    ...
    socket.on("entry", async (channel, nickName) => {
      // socket.join("채팅방이름");
      // socket.join("채팅방이름");은 서버 환경에 해당 채팅방 이름으로 채널을 개설하고 현재 입장하는 사용자를 해당 채팅방 사용자로 등록처리한다.
      ...
      socket
        .to(channel)
        .emit("entryOk", `${nickName} 님이 ${channel} 채널에 입장하셨습니다.`);

      // 현재 채널에 입장하고 있는 사용자에게만 메시지를 발송한다.
      // socket.emit(); 은 현재 서버소켓을 호출한(입장하는) 사용자에게만 메시지 발송하기
      socket.emit(
        "entryOk",
        `${nickName} 대화명으로  ${channel} 채널에 입장하였습니다.`
      );
    });

    // 채팅방 기준으로 해당 채팅방의 나를 포함한 모든 사용자들에게 메시지 전송
    // 클라이언트에서 메시지 데이터를 JSON 포맷으로 전송
    socket.on("channelMsg", async (msgData) => {
      // 클라이언트로 보낼 메시지 포맷 정의하기
      const message = `${msgData.nickName}: ${msgData.message}`;

      // io.to("채널명").emit();은 메시지를 보낸 당사자(나)를 포함한 현재 채널의 모든 사용자에게 메시지 전송
      io.to(msgData.channel).emit("receiveChannel", message);
    });

    // 접속한 채팅방 명시적 퇴장하기
    socket.on("exit", async (channel, nickName) => {
      // 나를 제외한 채팅방 내 모든 사용자에게 퇴장사실알림처리
      socket.to(channel).emit("exitOk", `${nickName} 님이 퇴장하셨습니다.`);

      // socket.leave(채팅방명); : 해당 채팅방 나가기 처리
      socket.leave(channel); // 채팅방 퇴장 처리
