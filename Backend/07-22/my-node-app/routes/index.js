...
router.get("/", function (req, res, next) {
  console.log(
    "https://localhost:3000/라우팅메소드 의 콜백 함수가 호출되었습니다."
  );

  console.log(
    "사용자 클라이언트 정보를 req(HttpRequest) 객체에서 조회 가능합니다.",
    req
  );

  // 테스트 쿼리스트링 아이디값을 추출합니다.
  // https://localhost:3000/?id=testing&stoc=2
  const testId = req.query.id;
  const stock = req.query.stock;

  console.log("URL QueryString 방식으로 전달된 데이터 출력:", testId, stock);
  console.log(
    "서버측에서 웹브라우저 응답 처리를 위한 res(HttpResponse) 객체 출력하기",
    res
  );
  console.log(
    "res.render() 메서드를 통해 메인 뷰파일(index.ejs) 파일이 바로 호출됩니다."
  );
...
// 콜백함수는 해당 라우팅메소드가 호출되면 서버에서 응답처리하기 위한 함수가 자동 실행.
