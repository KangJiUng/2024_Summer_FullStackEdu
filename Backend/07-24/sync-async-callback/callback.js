...
// 아주 간단한 계산함수 만들기
// fnPlus(a, b) 함수에 전달되는 a, b 파라미터(매개변수)를 받아 로직(더하기)을 처리하고 결과값을 반환하는 함수
function fnPlus(a, b) {
  let c = a + b;
...
// 콜백함수 개념이해하기
// 콜백함수를 이용하면 특정함수 내에서 순차적 프로그래밍이 가능합니다.
...
// 해당 결과값을 콘솔을 통해 출력해주는 로깅 함수
function fnLogging(result) {
  console.log(`계산 결과값은 ${result}입니다.`);
}

// 이전 함수의 계산된 결과값에 기본 배송비 3000원을 추가한 로직으로, 콘솔 출력
...
// 자바스크립트 함수는 특정 함수에 파라미터(매개변수)로 전달 가능합니다.
// fnLogging() 함수를 콜백함수로 전달
const result0 = fnPlusCallBack(1000, 2000, fnLogging);

// 콜백함수를 파라미터 내에서 직접 정의 전달하기
// const result0 = fnPlusCallBack(1000, 2000, fnLogging); 와 동일한 방식
const result1 = fnPlusCallBack(1000, 2000, function (result) {
  console.log(`계산 결과값은 ${result}입니다.`);
});
...
// 전달된 콜백함수를 순차적(동기적)으로 실행될 수 있게 강제하기 위한 수단으로 사용됩니다.
