...
const TodolistAll = () => {
  ...
  // 할일 저장 이벤트 처리 함수
  const saveTodo = () => {
    // 단일 할일 정보(todo)를 할일 목록 배열(todos)에 추가하기
    // setTodos("신규배열목록");
    // 기존 배열 목록의 복사본을 생성하고 해당 복사본 배열에 신규 아이템을 추가한다.
    setTodos([...todos, todo]);
  ...
  const removeItem = (
    ...
    // todos 배열에서 삭제하려는 index 값과 일치하지 않는 모든 목록을 반환한다.
    const filterdTodos = todos.filter((item: string, i: number) => i !== index);

    // 필터링된 배열 목록으로 다시 할일 목록 상태값을 갱신한다.
