...
export default async function handler(
  ...
  try {
    ...
    if (req.method == "POST") {
      ...
      // Step3-3(embeding/embeding 저장 과정): Splitting된 문서내 단어들을 임베딩(벡터화처리)처리해서 메모리벡터저장소에 저장하기
      // MemoryVectorStore.fromDocuments(임베딩된문서,사용할 임베딩모델 처리기);
      // 지정한 임베딩 모델을 통해 chunk data를 개별 vector 수치화하고 이 수치화된 데이터를 지정한 vector 전용 저장소에 저장한다.
